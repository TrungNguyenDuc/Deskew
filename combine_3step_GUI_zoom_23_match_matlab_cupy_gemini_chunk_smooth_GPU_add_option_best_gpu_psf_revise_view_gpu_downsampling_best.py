            ...
            if zm_f<1.0:
                ...
                try:
                    if gpu_chunks == 1:
                        ...
                        clear_gpu_memory()
                        log_message_gui(f"Deskew:GPU Z downsampling successful (single chunk). Shape={final_image_for_mips_and_save.shape}", log_queue)
                        chunk_success = True
                    else:
                        ...
                        for i, chunk_z_idx in enumerate(chunk_indices):
                            ...
                            log_message_gui(f"  Processing GPU chunk {i+1}/{gpu_chunks} (Z={z_start}-{z_end-1})...", log_queue)
                            ...
                            try:
                                ...
                                log_message_gui(f"  GPU chunk {i+1} failed: {type(e_chunk).__name__} - {e_chunk}", log_queue, "ERROR")
                        ...
                        if processed_chunks:
                            ...
                            log_message_gui(f"Deskew:GPU Z downsampling successful ({gpu_chunks} chunks). Final shape={final_image_for_mips_and_save.shape}", log_queue)
                            chunk_success = True
                        
                except Exception as e_gpu:
                    log_message_gui(f"Deskew:GPU downsampling failed ({type(e_gpu).__name__}), falling back to CPU", log_queue, "WARNING")
                    clear_gpu_memory()
                
                # Fall back to CPU if GPU failed
                if not chunk_success:
                    log_message_gui("Deskew:Attempting CPU downsampling...", log_queue)
                    try:
                        final_image_for_mips_and_save = scipy_zoom(final_image_to_process, (1.0, 1.0, zm_f),
                                                                  order=1, mode='constant', cval=0.0, prefilter=True)
                        log_message_gui(f"Deskew:CPU Z downsampling complete. Shape={final_image_for_mips_and_save.shape}", log_queue)
                    except Exception as e_cpu:
                        log_message_gui(f"Deskew:CPU downsampling failed! Keeping original size. Error: {e_cpu}", log_queue, "ERROR")
                        ds_f_z = 1
                        final_dz_eff_um = dx_um
                        final_image_for_mips_and_save = final_image_to_process
            else: 
                log_message_gui(f"Deskew:Zoom factor {zm_f:.3f}>=1.0, skip Z downsample.", log_queue)
                ds_f_z = 1
                final_dz_eff_um = dx_um
...
class OPMAnalysisApp:
    ...
    def _clear_displayed_image_gui(self):
        ...
        if self.original_pil_image:
            try:
                ...
                # Optional: log this action if your log_queue is accessible here or pass self.log_queue
                # log_message_gui("Closed previous MIP image handle.", self.log_queue, "DEBUG")
            except Exception as e_close:
                print(f"Warning: Error closing previous MIP image: {e_close}") # Or log it
        # --- Modification Ends ---
    ...
    def _clear_displayed_psf_plot_gui(self):
         # --- Modification Starts ---
        if self.psf_plot_original_pil_image:
            try:
                self.psf_plot_original_pil_image.close()
                # log_message_gui("Closed previous PSF plot image handle.", self.log_queue, "DEBUG")
            except Exception as e_close:
                print(f"Warning: Error closing previous PSF plot image: {e_close}")
        # --- Modification Ends ---
    ...
    def _setup_initial_image_for_display_gui(self, image_path, autofit=False):
        # --- Modification Starts: Close existing image first ---
        if self.original_pil_image:
            try:
                self.original_pil_image.close()
                # log_message_gui("Closed existing MIP before loading new one.", self.log_queue, "DEBUG")
            except Exception as e_close:
                print(f"Warning: Error closing existing MIP: {e_close}")
            self.original_pil_image = None
            self.original_image_data_np = None # Also clear the numpy array
        # --- Modification Ends ---
    ...
    def _setup_initial_psf_plot_for_display_gui(self, image_path, autofit=False):
        ...
        if self.psf_plot_original_pil_image:
            try:
                self.psf_plot_original_pil_image.close()
                # log_message_gui("Closed existing PSF plot before loading new one.", self.log_queue, "DEBUG")
            except Exception as e_close:
                print(f"Warning: Error closing existing PSF plot: {e_close}")
            self.psf_plot_original_pil_image = None
        # --- Modification Ends ---
